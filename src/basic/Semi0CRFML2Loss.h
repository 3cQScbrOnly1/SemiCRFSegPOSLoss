#ifndef _Semi0CRFML2LOSS_H_
#define _Semi0CRFML2LOSS_H_

#include "N3L.h"

using namespace Eigen;

struct Semi0CRFML2Loss{
public:
	int labelSize;
	int clabelSize;
			...
			for (int i = 0; i < labelSize; ++i) {
				...
				for (int dist = 0; dist < seq_size - idx && dist < maxLens[i]; dist++) {
					...
					if (idx == 0) {
						...
						maxScores[idx][dist][i] = x[idx][dist]->val(i, 0) * lambda + cx[idx][dist]->val(clabeli, 0) * (1 - lambda) + maxscore;
