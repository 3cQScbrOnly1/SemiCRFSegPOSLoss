...
public:
	...
	vector<LinearNode> coutput;

	NRMat<PNode> poutput; //use to store pointer matrix of outputs
	NRMat<PNode> pcoutput; //use to store pointer matrix of coutputs
	int max_seg_length;
	int type_num;
...
public:
	inline void initial(LookupTable& words, vector<LookupTable>& types, UniParams& tanh1_project, LSTM1Params& left_lstm_project,
		...
		BiParams& segtanh_project, UniParams& olayer_linear, UniParams& colayer_linear, int wordcontext, dtype dropout){
		...
		for (int idx = 0; idx < output.size(); idx++){
			...
			coutput[idx].setParam(&colayer_linear);
		}		
	}
...
public:
	...
	inline void forward(const vector<Feature>& features, bool bTrain = false){
		...
		pcoutput.resize(seq_size, max_seg_length);
		poutput = NULL;
		pcoutput = NULL;
		offset = 0;
		for (int idx = 0; idx < seq_size; idx++) {
			...
			for (int dist = 0; idx + dist < seq_size && dist < max_seg_length; dist++) {
				...
				coutput[offset + dist].forward(this, &(seg_hidden1_drop[offset + dist]));
...
public:
	...
	inline void init(int wordcontext, int charcontext, int hiddensize1, int rnnhiddensize, int hiddensize2, int seghiddensize) {
		if (_words.nVSize <= 0 || _loss.labelSize <= 0 || _loss.clabelSize <= 0 || _loss.maxLen <= 0 || _segs.nVSize <= 0){
		...
		_labelSize = _loss.labelSize;
		_clabelSize = _loss.clabelSize;
		...
		_colayer_linear.initial(_clabelSize, _seghiddensize, false, 800);
		...
		_colayer_linear.exportAdaParams(_ada);
		...
		_pcg->initial(_words, _types, _tanh1_project, _left_lstm_project, _right_lstm_project, _tanh2_project, _seglayer_project, _segs, _segtanh_project, _olayer_linear, _colayer_linear, _wordcontext, _dropOut);
	...
	inline dtype train(const vector<Example>& examples, int iter) {
		...
		for (int count = 0; count < example_num; count++) {
			...
			//}
			cost += _loss.loss(_pcg->poutput, _pcg->pcoutput, example.m_seglabels, _eval, _eval, example_num);
	...
	inline void predict(const vector<Feature>& features, NRMat<int>& results) {
		...
		//}
		_loss.predict(_pcg->poutput, _pcg->pcoutput, results);
	}
	...
	inline dtype cost(const Example& example){
		...
		//}
		cost += _loss.cost(_pcg->poutput, _pcg->pcoutput, example.m_seglabels, 1);
