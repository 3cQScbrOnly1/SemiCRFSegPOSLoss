...
public:
	inline void initial(LookupTable& words, vector<LookupTable>& types, UniParams& tanh1_project, LSTM1Params& left_lstm_project,
		...
		UniParams& colayer_linear, int wordcontext, dtype dropout){
	...
	inline void forward(const vector<Feature>& features, bool bTrain = false){
		...
		for (int idx = 0; idx < seq_size; idx++) {
			...
			for (int dist = 0; idx + dist < seq_size && dist < max_seg_length; dist++) {
				...
				coutput[offset + dist].forward(this, &(outputseg[offset + dist]._output_drop));
...
public:
	...
	inline void init(int wordcontext, int charcontext, int hiddensize1, int rnnhiddensize, int hiddensize2, int seghiddensize) {
		if (_words.nVSize <= 0 || _loss.labelSize <= 0 || _loss.clabelSize <= 0 || _loss.maxLen <= 0){
		...
		_pcg->initial(_words, _types, _tanh1_project, _left_lstm_project, _right_lstm_project, _tanh2_project, _seglayer_project, _olayer_linear, _colayer_linear, _wordcontext, _dropOut);
