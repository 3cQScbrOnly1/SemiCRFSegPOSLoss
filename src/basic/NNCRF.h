...
public:
	inline void initial(LookupTable& words, vector<LookupTable>& types, UniParams& tanh1_project, LSTM1Params& left_lstm_project,
		LSTM1Params& right_lstm_project, BiParams& tanh2_project, UniParams& olayer_linear, UniParams& colayer_linear,
...
public:
	...
	inline void forward(const vector<Feature>& features, bool bTrain = false){
		...
		for (int idx = 0; idx < seq_size; idx++) {
			...
			coutput[idx].forward(this, &(word_hidden2_drop[idx]));
		}
	}
...
public:
	...
	UniParams _colayer_linear; // coutput
...
public:
	...
	inline void init(int wordcontext, int charcontext, int hiddensize, int rnnhiddensize) {
		if (_words.nVSize <= 0 || _loss.labelSize <= 0 || _loss.clabelSize <= 0){
			std::cout << "Please initialize embeddings and loss functions before this" << std::endl;
			return;
		}
		...
		_colayer_linear.initial(_clabelSize, _hiddensize, false, 700);
		...
		_pcg->initial(_words, _types, _tanh1_project, _left_lstm_project, _right_lstm_project, _tanh2_project, 
			_olayer_linear, _colayer_linear, _wordcontext, _dropOut);
	...
	inline dtype train(const vector<Example>& examples, int iter) {
		...
		for (int count = 0; count < example_num; count++) {
			...
			//}
			cost += _loss.loss(getPNodes(_pcg->output, seq_size), getPNodes(_pcg->coutput, seq_size), example.m_labels, _eval, _eval, example_num);
	...
	inline void predict(const vector<Feature>& features, vector<int>& results) {
		...
		//}
		_loss.predict(getPNodes(_pcg->output, seq_size), getPNodes(_pcg->coutput, seq_size), results);
	}
	...
	inline dtype cost(const Example& example){
		...
		//}
		cost += _loss.cost(getPNodes(_pcg->output, seq_size), getPNodes(_pcg->coutput, seq_size), example.m_labels, 1);
