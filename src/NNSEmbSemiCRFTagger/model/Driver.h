...
public:
	...
	inline dtype train(const vector<Example>& examples, int iter) {
		...
		for (int count = 0; count < example_num; count++) {
			...
			//cost += _model_params._loss.loss(&(_pcg->output[idx]), example.m_labels[idx], _eval, example_num);				
			//}
			cost += _model_params._loss.loss(_pcg->poutput, _pcg->pcoutput, example.m_seglabels, _eval, _eval, example_num);
	...
	inline void predict(const vector<Feature>& features, NRMat<int>& results) {
		...
		//for (int idx = 0; idx < seq_size; idx++) {
		//	_model_params._loss.predict( &(_pcg->output[idx]), results[idx]);
		//}
		_model_params._loss.predict(_pcg->poutput, _pcg->pcoutput, results);
	}
	...
	inline dtype cost(const Example& example){
		...
		//loss function
		//for (int idx = 0; idx < seq_size; idx++) {
		//	cost += _model_params._loss.cost(&(_pcg->output[idx]), example.m_labels[idx], 1);
		//}
		cost += _model_params._loss.cost(_pcg->poutput, _pcg->pcoutput, example.m_seglabels, 1);
