...
void Tagger::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile) {
	...
	maxLabelLength.resize(m_driver._model_params._seg_label_alpha.size());
	...
	for (int j = 0; j < m_driver._model_params._seg_label_alpha.size(); j++){
		std::cout << "max length of label " << m_driver._model_params._seg_label_alpha.from_id(j) << ": " << maxLabelLength[j] << std::endl;
	}
	m_driver._model_params._loss.initial(m_driver._model_params._label_alpha.size(), m_driver._model_params._clabel_alpha.size(), m_label_f2c, m_options.tuneLambda, 1000);
