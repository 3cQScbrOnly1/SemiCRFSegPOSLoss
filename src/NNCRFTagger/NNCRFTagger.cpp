...
void Tagger::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile) {
	...
	m_classifier._loss.initial(m_labelAlphabet.size(), m_clabelAlphabet.size(), m_label_f2c, m_options.tuneLambda, 1000);
	...
	m_classifier.init(m_options.wordcontext, m_options.charcontext, m_options.hiddenSize, m_options.rnnHiddenSize);
